...
function extractRowNumberFromUpdatedRange(
...
/**
 * Search a single column (bottom-up) for an exact cell value; returns 1-based row number.
 */
async function findRowByColumnValue(
  sheetId: string,
  columnRange: string,
  searchValue: string,
  label: string
...
async function safeFindExistingRow(
  findFn: () => Promise<number | undefined>,
  label: string
): Promise<number | undefined> {
  try {
    return await findFn();
    ...
    if (isTransientSheetsError(error)) {
      logger.warn({ label, error }, 'Idempotency check failed with transient error');
      return undefined;
    }
    throw error;
  }
...
/**
 * Retry append on transient errors, but verify the row was not already written first.
 * Prevents duplicate rows when the server applies the append but the client sees Premature close.
 */
async function withIdempotentAppendRetry(
  appendFn: () => Promise<{ data: { updates?: { updatedRange?: string | null } } }>,
  findExistingRow: () => Promise<number | undefined>,
  label: string
): Promise<number | undefined> {
  const maxAttempts = 5;
  let lastError: unknown;
  ...
  for (let attempt = 1; attempt <= maxAttempts; attempt++) {
    try {
      const response = await appendFn();
      return extractRowNumberFromUpdatedRange(response.data.updates?.updatedRange);
    } catch (error) {
      lastError = error;
      if (!isTransientSheetsError(error)) {
        throw error;
      ...
      const existingRow = await safeFindExistingRow(findExistingRow, label);
      if (existingRow !== undefined) {
        logger.warn(
          { attempt, label, existingRow },
          'Append succeeded on server despite client error; skipping retry'
        );
        return existingRow;
      }
      ...
      logger.warn(
        { attempt, maxAttempts, delayMs, label, error },
        'Transient Sheets append error, retrying'
      );
      await new Promise((resolve) => setTimeout(resolve, delayMs));
    }
  }
...
export async function appendRow(chatId: number, row: SheetRow): Promise<number | undefined> {
  ...
  const rowNumber = await withIdempotentAppendRetry(
    () =>
    ...
    () => findRowByColumnValue(sheetId, 'Invoices!J:J', row.drive_link, 'appendRow:verify'),
    'appendRow'
  );
...
/**
 * Column headers for the Generated Invoices sheet (14 columns)
 */
export const GENERATED_INVOICES_HEADERS = [
  ...
  'Generated At',
  'PDF Link',
  // New columns (L-N) - appended at end for backward compatibility
  'Currency',
  'Related Invoice',
  'Input Method',
];
...
async function createGeneratedInvoicesTab(sheetId: string): Promise<void> {
  ...
  await withSheetsRetry(
    () =>
      sheets.spreadsheets.values.update({
        spreadsheetId: sheetId,
        range: `'${GENERATED_INVOICES_TAB}'!A1:N1`,
        valueInputOption: 'RAW',
        requestBody: {
  ...
  const rowNumber = await withIdempotentAppendRetry(
    () =>
      sheets.spreadsheets.values.append({
        ...
        range: `'${GENERATED_INVOICES_TAB}'!A:N`, // Columns A through N (14 columns)
        valueInputOption: 'USER_ENTERED',
        insertDataOption: 'INSERT_ROWS',
        requestBody: {
          values,
    ...
    () =>
      findRowByColumnValue(
        resolvedSheetId,
        `'${GENERATED_INVOICES_TAB}'!A:A`,
        row.invoice_number,
        'appendGeneratedInvoiceRow:verify'
      ),
